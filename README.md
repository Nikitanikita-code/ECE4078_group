...
update:
    the motor function was imepemented by adjesting the r_val and l_val of the motor

      if command[1] is 0, the car doens't turn as there's no offset
      setting the value with +- ve to add offset, allowing the car to turn 
        
     the code uses conditional statement to see what keyboard is pressed, and from there it change the speed of the wheels accordingly

    a sprint function is also imprmented by increasing the commend[0] and leaving commend[1] alone
    to check the current state of the robort, a variable currentState is created to see if the robot is sprinting
